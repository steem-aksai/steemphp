...
class OperationSerializer
{
     ...
     * @throws \Exception
     */
    public static function serializeTransaction($trx, $byteBuffer = null)
    {
        $buffer = $byteBuffer === null ? (new ByteBuffer()) : $byteBuffer;

        $buffer->writeInt16LE($trx['ref_block_num'], 0);
        $buffer->writeInt32LE($trx['ref_block_prefix'], 2);
        $expirationSec = is_int($trx['expiration']) ? $trx['expiration'] : strtotime($trx['expiration']);
        $buffer->writeInt32LE($expirationSec, 6);
        $buffer->writeInt8(count($trx['operations']));


        //serialize only operations data
        foreach ($trx['operations'] as $operation) {
            $opData = $operation[1];
            self::serializeOperation($operation[0], $opData, $buffer);
        }


        $buffer->writeInt8(count($trx['extensions']));
        foreach ($trx['extensions'] as $extansion) {
            //is it used anywhere?
        }
    ...
    public static function serializeOperation($operationName, $data, $byteBuffer)
    {
        ...
        foreach (self::getOpFieldsTypes($operationName) as $field => $type) {
